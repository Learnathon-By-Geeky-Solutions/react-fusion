          ...
          <div key={milestone.id} className="mb-3">
            ...
            <button
              ...
              className={`font-medium text-gray-800 px-3 py-2.5 rounded-md w-full text-left flex justify-between items-center transition-colors ${
                openMilestones[milestone.id] ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 hover:bg-gray-300'
              }`}
            >
            ...
            <div
              className={`mt-2 pl-2 transition-all duration-300 ${
                openMilestones[milestone.id] ? 'max-h-full opacity-100' : 'max-h-0 opacity-0 overflow-hidden'
              }`}
            >
              {milestone.modules.map((module, modIndex) => (
                <div key={module.id} className="bg-white rounded-md mb-2 shadow-sm border border-gray-200">
                  {/* Module Toggle */}
                  <button
                    onClick={() => toggleModule(module.id)}
                    className={`text-sm font-medium px-3 py-2.5 w-full text-left flex justify-between items-center transition-colors ${
                      openModules[module.id] ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                    <span className="flex items-center">
                      <span className="text-gray-500 mr-1.5">{mIndex + 1}.{modIndex + 1}</span>
                      {module.title}
                    </span>
                    <svg
                  ...
                  <div
                    className={`transition-all duration-300 ${
                      openModules[module.id] ? 'max-h-full opacity-100 pb-2' : 'max-h-0 opacity-0 overflow-hidden pb-0'
                    }`}
                  >
                    {module.videos.map((video, vIndex) => (
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span className="text-xs font-semibold mr-1.5">{mIndex + 1}.{modIndex + 1}.{vIndex + 1}</span>

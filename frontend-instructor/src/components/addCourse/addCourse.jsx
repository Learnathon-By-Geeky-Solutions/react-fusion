  ...
  const handleSubmit = async (values, { setSubmitting, setStatus }) => {
    try {
      ...
      if (result.success) {
        ...
        setStatus({
          error: `Failed: ${result.message || 'Unknown error'}`
        });
              ...
              {({ push: pushMilestone, remove: removeMilestone }) => (
                <div>
                  {values.milestones.map((milestone, milestoneIndex) => (
                    <div
                      key={milestoneIndex}
                      className='mt-4 p-4 bg-gray-600 text-white rounded-lg'
                    >
                      ...
                      {/* Modules */}
                      <FieldArray
                        name={`milestones[${milestoneIndex}].modules`}
                      >
                          ...
                          <div>
                            {milestone.modules &&
                              milestone.modules.map((module, moduleIndex) => (
                                <div
                                  key={moduleIndex}
                                  className='mt-2 p-3 bg-gray-400 text-black rounded-lg'
                                >
                                  <div className='flex justify-between items-center'>
                                  ...
                                  <div className='mt-3'>
                                    ...
                                    <FieldArray
                                      name={`milestones[${milestoneIndex}].modules[${moduleIndex}].videos`}
                                    >
                                      {({
                                        push: pushVideo,
                                        remove: removeVideo
                                          ...
                                          {module.videos &&
                                            module.videos.map(
                                              (video, videoIndex) => (
                                                <div
                                                  key={videoIndex}
                                                  className='mt-2 p-3 bg-gray-200 text-black rounded-lg'
                                                >
                                                  <div className='flex justify-between items-center'>
